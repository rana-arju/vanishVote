...
import { Plus, Trash2, Clock, Eye, Lock } from "lucide-react";
...
const CreatePollForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Create the poll
    createPoll(pollData).then((newPoll) => {
...
${pollType === "multiple-choice" && "bg-secondary text-secondary-foreground"}`}
        ...
        <div className="pt-4 w-full">
          <button className="bg-primary w-full  text-white px-5 py-2 rounded-lg shadow-md hover:bg-primary/90 transition cursor-pointer">
            Create Poll
          </button>
        </div>
      </form>
